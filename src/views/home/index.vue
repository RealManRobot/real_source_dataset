<template>
  <div class="rel ovya pb100" style="background-color: #e7ecf3;">
    <div class="gf rel blankBg" style="background:linear-gradient(135deg, #1e293b 0%, #334155 100%);padding:60px 30px 120px">
      ...
      <div class="auto rds30 f ac mt30 fd pl10 pr5" style="width:1200px;background:#131c2f;">
    ...
    <!--下面-->
    <div class="w80 auto pt30r">
      <div id="sourceAddress" class="f ac fs30r b pb10r" style="">{{$t('open_source_address')}}</div>
      <div class="f ac xs mb30r">
      ...
      <!-- <div id="partnerCompany" class="f ac xc fs24r b pt20r pb20r">合作公司</div>
      <div class="f ac xc">
        <img v-lazyImg="damoyuan_logo" style="width:200px;" alt="">
        <img v-lazyImg="damoyuan_logo" style="width:200px;" alt="">
        <img v-lazyImg="damoyuan_logo" style="width:200px;" alt="">
      </div> -->
      <!--核心优势-->
      <div id="mainAdvance" class="f ac fs30r b pb10r" style="">{{$t('core_strengths')}}</div>
      <div class="fs16r f ac mb40r" style="color:#4b5563;">{{ $t('core_strengths_desc') }}</div>
      <div class="f xb rw auto mb50" style="gap: 20px 0px;">
        <div v-for="item in mainAdvantageList" :key="item.id" class="trans3 poi rds10 hoverShadow pl30r pr30r pt30r pb30r bgf borderBox" style="width: clamp(290px, 32%, 500px)">
          <div class="rds5 f ac xc mb20r" :style="{width:'60px',height:'60px',background: item.background}">
      ...
      <!--完整数据模态-->
      <div id="dataModality" class="f ac fs30r b pb10r">{{$t('complete_data_modality')}}</div>
      <div class="fs16r f ac mb30r" style="color:#4b5563">{{$t('complete_data_modality_desc')}}</div>
      <div class="f xb rw auto mb50" style="gap:20px;">
        <div v-for="item in dataModalList" :key="item.id" class="trans3 poi rds10 hoverShadow pl30r pr30r pt30r pb30r bgf borderBox" style="width: clamp(240px, 22%, 360px)">
          <div class="f ac xc">
            <div class="rds50 f ac xc" :style="{width:'70px', height:'70px',background: item.background}">
              <atomSvg :type="item.svg" width="28" height="28" :color="item.svgColor"></atomSvg>
            </div>
          </div>
          <div class="fs18 b f ac xc mt10r mb10r">{{item.name}}</div>
          <div class="fs14 f ac xc mt10r mb10r g6 wrap" style="height:70px">{{ item.desc }}</div>
          <div class="f xc ac g6">
            <div class="rds50 mr5" :style="{width:'14px',height:'14px',background: item.tipbg}"></div>
            <div>{{item.tip}}</div>
          </div>
        </div>
      ...
      <div class="fs16r dib mb30r poi hover1890ff g4b5563 borderBox" @click="goUrl('https://develop.realman-robotics.com/datatraining/guide/datacollectionrobot/')">
        <div class="f">
          <atomSvg type="tip" width="24" height="24" color="#1890ff"></atomSvg>
          {{ $t('robot_collection_plat_desc') }}
        </div>
      </div>
      <div class="trans3 rds10 fs16 borderBox mb50" >
        <div class="f rw xb" style="gap: 20px 20px;">
          <div class="rds20" style="width:280px;background: linear-gradient(to bottom, #195DE3 0%, #A2BDEC 100%);">
            ...
            <img src="@/assets/RS01.png" style="width:192px;height:411px;margin-left:54px;" class="" alt="">
          </div>
          <div class="rds20" style="width:280px;background: linear-gradient(to bottom, #195DE3 0%, #A2BDEC 100%);">
            <div class="fs20 b mb5 gf f ac xc pt20">RS-02</div>
            <div class="fs20 b mb5 gf f  xc mb30 pl10 pr10 wrap" style="min-height:80px;">{{$t('Standard_version_both_arm_lift')}}</div>
            <img src="@/assets/RS02.png" style="width:192px;height:411px;margin-left:40px;" class="" alt="">
          </div>
          <div class="rds20" style="width:280px;background: linear-gradient(to bottom, #195DE3 0%, #A2BDEC 100%);">
            <div class="fs20 b mb5 gf f ac xc pt20">RS-03</div>
          ...
          <div class="rds20 borderBox pt20 pb10 pl30 pr30 f1" style="min-width:400px;background: linear-gradient(to bottom, #b6cdf4 0%, #f1f9ff 100%);;">
            ...
            <div class="g6 fs16 pb20" style="border-bottom:1px solid #fff;">{{ $t('multimodal_visual_perception_desc') }}</div>
            <div class="g0 fs24 f ac xc pt10 pb10">{{$t('integrated_six_dimensional_force_sensing')}}</div>
            <div class="g6 fs16 pb20" style="border-bottom:1px solid #fff;">{{ $t('integrated_six_dimensional_force_sensing_desc') }}</div>
            <div class="g0 fs24 f ac xc pt10 pb10">{{$t('end_of_generalization_operation')}}</div>
            <div class="g6 fs16 pb20" style="border-bottom:1px solid #fff;">{{ $t('end_of_generalization_operation_desc') }}</div>
            <div class="g0 fs24 f ac xc pt10 pb10">{{$t('spatial_perception')}}</div>
            <div class="g6 fs16 pb20" style="border-bottom:1px solid #fff;">{{ $t('spatial_perception_desc') }}</div>
          </div>
        </div>
      </div>
      <!--遥操作-->
      <div id="remoteOperaction" class="f ac fs30r b mt50 pb10r">{{$t('Teleoperation')}}</div>
      <img src="@/assets/remote_operation.png" style="width:720px;height:405px;" class="db auto rds10" alt="" />
      <div class="fs16r f ac mb30r  mb40r" style="color:#4b5563">{{$t('three_major_advantages')}}</div>
      <div class="f rw xb auto" style="gap:20px 0;">
        <div v-for="item in remoteAdvantageList" :key="item.id" class=" trans3 poi bgf ovh rds10 hoverShadow" style="width: clamp(320px, 32%, 500px)">
          <div class="f ac pl20r pr30r pt15r pb15r" :style="{background: item.background}">
            ...
            <div class="b fs16 f1">{{ item.name }}</div>
          </div>
